...
class Vector:
    ...
    def get_unit_vector(self, p=True):
        new_values = []
        for i in range(0, self.dimensions):
            new_values.append(self.values[i] / self.get_length(p=False))
        n = Vector(*new_values)
        if p:
            n.print_summary()
        return n
    ...
    def get_interior_angle(self, other, radians=True):
        if self.dimensions != other.dimensions:
            raise ValueError("Vectors must have same Dimensions")
        theta = np.arccos(self.get_dot_product(other) / (self.get_length(p=False) * other.get_length(p=False)))
        if radians:
            print(theta)
            return theta
        else:
    ...
    def get_scalar_projection(self, other, p=True):
        if self.dimensions != other.dimensions:
            raise ValueError("Vectors must have same Dimensions")
        sp = self.get_dot_product(other) / other.get_length(p=False)
        if p:
            print(sp)
        return sp
    ...
    def get_vector_projection(self, other):
        if self.dimensions != other.dimensions:
            raise ValueError("Vectors must have same Dimensions")
        uv = other.get_unit_vector(p=False)
        scalar = self.get_scalar_projection(other, p=False)
        v = uv.scale(scalar, p=False)
        v.print_summary()
        return v
